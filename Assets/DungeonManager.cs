...
public class DungeonManager : MonoBehaviour {
	...
	private GameObject[,] dungeonPositionsFloor;
	...
	public class SubDungeon{
		...
		public bool Split(int minRoomSize, int maxRoomSize) {
			...
			bool splitH;
			if (rect.width / rect.height >= 0.45) {
				splitH = false;
			} else if (rect.height / rect.width >= 0.45) {
				splitH = true;
			} else {
		...
		if (subDungeon.Leaf()) {
			for (int i = (int)subDungeon.room.x; i < subDungeon.room.xMax; i++) {
				for (int j = (int)subDungeon.room.y; j < subDungeon.room.yMax; j++) {
					GameObject instance = Instantiate (floorTile, new Vector3 (i, j, 0.0f), Quaternion.identity) as GameObject;
					instance.transform.SetParent (transform);
					dungeonPositionsFloor [i, j] = instance;
				}
			}
		} else {
		...
		foreach (Rect corridor in subDungeon.corridors) {
			for (int i = (int)corridor.x; i < corridor.xMax; i++) {
				for (int j = (int)corridor.y; j < corridor.yMax; j++) {
					if (dungeonPositionsFloor[i,j] == null) {
	...
	void Start () {
		...
		dungeonPositionsFloor = new GameObject[Rows, Columns];
